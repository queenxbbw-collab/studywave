...
const SORT_OPTIONS = [
  ...
  { value: "oldest", label: "Cele mai vechi", icon: Clock },
  { value: "most-voted", label: "Cele mai votate", icon: ThumbsUp },
  { value: "unsolved", label: "Nerezolvate", icon: CircleDot },
];
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-2xl font-extrabold text-foreground tracking-tight">Intrebari</h1>
          <p className="text-sm text-muted-foreground mt-1">
            {data ? (
              <span><strong className="text-foreground">{data.total.toLocaleString()}</strong> intrebari publicate</span>
            ) : "Se incarca..."}
          </p>
        </div>
        {user && (
          <Link href="/ask">
            <Button className="gradient-primary text-white border-0 gap-2 h-9 px-4 rounded-xl font-semibold shadow-sm hover:opacity-90">
              <Plus className="h-4 w-4" strokeWidth={2.5} /> Pune intrebare
            </Button>
          </Link>
        )}
      </div>
          ...
          <form onSubmit={handleSearch} className="flex gap-2 flex-1">
            <div className="relative flex-1">
              ...
              <Input
                ...
                placeholder="Cauta intrebari, autori..."
                className="pl-10 h-9 rounded-lg border-border/70 bg-gray-50 focus-visible:bg-white"
              />
            </div>
            <Button type="submit" className="h-9 px-4 rounded-lg gradient-primary text-white border-0 font-semibold">
              Cauta
            </Button>
          </form>
          ...
          <div className="flex items-center gap-2 mt-3 pt-3 border-t border-border/50">
            ...
            {search && (
              <span className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full bg-primary/8 text-primary border border-primary/15">
                Cautare: "{search}"
                <button onClick={() => { setSearch(""); setInputValue(""); }} className="hover:text-primary/60">×</button>
              </span>
            )}
            {subject !== "all" && (
              <span className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full bg-primary/8 text-primary border border-primary/15">
                {subject}
                <button onClick={() => setSubject("all")} className="hover:text-primary/60">×</button>
              </span>
            )}
          </div>
        )}
      ...
      <div className="flex items-center gap-1 mb-4 bg-gray-100/70 p-1 rounded-xl w-fit">
        {[
          { label: "Recente", value: "newest", icon: Clock },
          { label: "Votate", value: "most-voted", icon: TrendingUp },
          { label: "Nerezolvate", value: "unsolved", icon: CircleDot },
        ].map(tab => (
          <button
            key={tab.value}
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 bg-white rounded-xl border border-border/60">
          ...
          <p className="text-sm text-muted-foreground mt-1">Incearca un alt termen sau subiect</p>
        </div>
      ) : (
        <>
          <div className="space-y-3">
          ...
          {data && data.totalPages > 1 && (
            <div className="flex items-center justify-between mt-8 pt-6 border-t border-border/50">
              <p className="text-sm text-muted-foreground">
                Pagina <strong>{page}</strong> din <strong>{data.totalPages}</strong>
              </p>
              <div className="flex items-center gap-2">
                ...
                >
                  Urmator <ChevronRight className="h-4 w-4" />
                </Button>
              </div>
            </div>
          )}
